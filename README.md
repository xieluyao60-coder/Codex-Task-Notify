...
1. 使用 `@parcel/watcher` 做全局发现，捕捉新增或变更的 session 文件。
2. 被发现或手动加入的 session 会进入热查询队列。
3. 热查询队列默认每 `5000ms` 检查一次文件的大小和修改时间。
4. 只有文件发生变化时，才继续读取 JSONL 并解析 `task_complete`。
5. 已通知过的事件会写入本地 state，避免重复通知。
...
- Windows 文件系统事件在长时间运行和递归监听场景下可能出现随机延迟。
- 当前版本仍依赖 `@parcel/watcher` 做全局发现，热查询负责降低已活跃 session 的延迟。
- 已归档 Codex 会话目前更像是移动到 `.codex/archived_sessions`，不是在原 session 文件中写入 `archived` 字段。
- 后续更稳的方向是实现纯 stat 轮询的新建/归档发现器，或针对 Windows 使用更底层的目录变更通知机制。
