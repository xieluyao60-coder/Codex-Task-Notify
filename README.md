...
- VS Code 扩展已经改为单一 `VS daemon` 架构：同一台机器上多个 VS Code 窗口会共享一份 watcher、共享一份 desktop / Bark / quota alert 发送链路。
- VS Code 扩展自身现在只负责 UI 与命令交互；真正的监视、冷热查询、余额刷新、系统级通知发送都在后台 `VS daemon` 进程里完成。
- CLI 目前仍保持现有本地 daemon 模式；“多个 CLI 终端共享一份 CLI daemon”属于下一阶段。
- 当前版本不处理 VS daemon 与 CLI daemon 之间的跨 surface 去重；如果 VS 与 CLI 同时运行，桌面通知和 Bark 仍可能重复。
...
- 扩展激活时会先连接本地 named pipe；如果 `VS daemon` 尚未存在，则自动拉起扩展自带的后台 Node 进程。
- `VS daemon` 独占 `CodexNotifyRuntime`、`CodexSessionWatcher`、Desktop/Bark/webhook channel，以及 quota alert 的主动发送。
- 每个 VS Code 窗口只保留一个 RPC client，并通过 pipe 与 `VS daemon` 通信。
- IDE 内置弹窗不再通过 runtime channel 直接发送，而是由 `VS daemon` 广播事件后，各个 VS 窗口本地显示。
- 当最后一个 VS Code 窗口断开时，`VS daemon` 会优雅关闭并释放 metadata 文件。
...
- 每次任务完成后都会从当前 session 再做一次快速额度刷新，并把结果持久化到本地，供多个窗口共享
- 本地会按账号分别记录额度快照；如果用户中途切换账号，新的刷新结果会写到新账号名下
...
Codex Task Notify: refresh balance
...
- `Usage Quota` 标题右侧有一个刷新按钮 `$(refresh)`，点击后会立刻读取当前 session 并快速刷新额度
