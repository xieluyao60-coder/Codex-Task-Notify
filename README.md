...
- **thread 可自由重命名**：你可以通过 CLI 命令或 VS Code 里的交互按钮，给每个正在进行的 Codex thread 随意重命名。后续所有通知都会优先使用这个重命名；当多个任务同时运行时，你能直接从通知里看出到底是哪一个任务完成了。这也变相解决了在 VS Code 中使用 Codex 时不支持为 thread 重命名的问题。
